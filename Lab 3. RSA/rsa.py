...
class RSA(object):
    ...
    def encrypt_string(self, string):
        res = ""
        for char in string:
            ch = self._crypt(ord(char), self.d)
            res += chr(ch)
        return res

    def encrypt_list(self, lst):
        res = []
        for elem in lst:
            el = self._crypt(elem, self.d)
            res.append(el)
        return res
    ...
    def decrypt_string(self, string):
        res = ""
        for char in string:
            ch = self._crypt(ord(char), self.e)
            res += chr(ch)
        return res

    def decrypt_list(self, lst):
        res = []
        for elem in lst:
            el = self._crypt(elem, self.e)
            res.append(el)
        return res
